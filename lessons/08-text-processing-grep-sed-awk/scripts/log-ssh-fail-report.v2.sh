#!/usr/bin/env bash
# Report SSH failed login source IPs with source/time/top filters.
...
usage() {
  cat <<'EOF'
Usage:
  log-ssh-fail-report.v2.sh [--source journal|auth] [--since STR] [--top N] [--all]

Examples:
  ./lessons/08-text-processing-grep-sed-awk/scripts/log-ssh-fail-report.v2.sh
