...
run "iam_policy_contract" {
  ...
  assert {
    condition = (
      aws_iam_role.github_actions_plan_role.name == "lab72-github-actions-plan-role" &&
      aws_iam_role.github_actions_apply_role.name == "lab72-github-actions-apply-role"
    )
  ...
  assert {
    ...
    error_message = "Lesson 72 must not reintroduce broad AWS managed policies into the normal plan/apply roles."
  }

  assert {
    condition = (
      strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab72/dev/full/terraform.tfstate") &&
      strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab72/dev/full/terraform.tfstate.tflock") &&
      !strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab68/dev/full/terraform.tfstate")
    )
    error_message = "Plan role backend policy must be scoped to the lesson 72 state key and lockfile only."
  }

  assert {
    condition = (
      strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab72/dev/full/terraform.tfstate") &&
      strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab72/dev/full/terraform.tfstate.tflock") &&
      !strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab68/dev/full/terraform.tfstate")
    )
    error_message = "Apply role backend policy must be scoped to the lesson 72 state key and lockfile only."
  }
  ...
  assert {
    condition = anytrue([
      ...
      try(
        ...
        statement.Resource == "arn:aws:iam::123456789012:role/lab72-ec2-ssm-role" &&
        statement.Condition.StringEquals["iam:PassedToService"] == "ec2.amazonaws.com",
        false
      )
    ])
    error_message = "Apply role PassRole must be restricted to the lab72 EC2 runtime role and ec2.amazonaws.com."
  }
