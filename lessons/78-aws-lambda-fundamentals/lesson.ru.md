...
1. Почему основную логику лучше отделять от handler?
2. Кто создаёт `event`, а кто создаёт `context`?
3. Почему нельзя использовать память execution environment как надёжное
...
1. Почему execution role не нужна permission `lambda:InvokeFunction` для вызова
   самой себя?
2. Что изменится в плане после изменения `lambda_function.py`?
...
1. Почему код `0` у AWS CLI ещё не доказывает успех handler?
2. Где искать Python stack trace?
3. Почему не стоит записывать весь входной event в CloudWatch Logs?
...
В этой лабе намеренно не добавляются VPC, NAT, DLQ и KMS только ради зелёного
результата сканера: это усложнило бы фундаментальный урок и добавило бы
AWS-стоимость. В рабочей среде такие требования оценивают по угрозам, типу
данных и сценарию отказа.
