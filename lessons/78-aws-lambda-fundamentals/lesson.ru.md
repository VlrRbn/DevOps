...
|---|---|---|
| `function` | функция Lambda | Код и конфигурация одного вычислительного ресурса Lambda |
| `runtime` | среда выполнения | Управляемая AWS среда языка, например `python3.14` |
| `handler` | handler, точка входа | Функция, которую Lambda вызывает для обработки события |
| `event` | событие | Входные данные одного вызова Lambda |
| `context` | контекст вызова | Объект AWS с `RequestId`, оставшимся временем и другими метаданными |
| `execution environment` | среда выполнения функции | Изолированная среда, в которой AWS запускает код Lambda |
| `invocation` | вызов функции | Одна попытка запуска handler с конкретным событием |
| `execution role` | роль выполнения Lambda | IAM role, которую принимает Lambda для доступа к AWS API во время работы |
| `cold start` | холодный запуск | Создание новой execution environment перед выполнением handler |
| `warm start` | тёплый запуск | Повторное использование уже подготовленной execution environment |
| `payload` | полезная нагрузка | Данные запроса или ответа Lambda |
| `FunctionError` | ошибка выполнения функции | Поле ответа Invoke API, показывающее необработанную ошибку handler |
