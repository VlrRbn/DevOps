mock_provider "aws" {
  ...
  mock_resource "aws_lb" {
    defaults = {
      arn      = "arn:aws:elasticloadbalancing:eu-west-1:123456789012:loadbalancer/app/lab70-app-alb/test"
      dns_name = "internal-lab70-app-alb.example.local"
    }
  }

  mock_resource "aws_lb_target_group" {
    defaults = {
      arn = "arn:aws:elasticloadbalancing:eu-west-1:123456789012:targetgroup/lab70-web-tg/test"
    }
  }
...
variables {
  ...
  tf_state_key         = "lab70/dev/full/terraform.tfstate"
}
...
run "iam_policy_contract" {
  ...
  assert {
    ...
    error_message = "Lesson 70 must not reintroduce broad AWS managed policies into the normal plan/apply roles."
  }

  assert {
    condition = (
      strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab70/dev/full/terraform.tfstate") &&
      strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab70/dev/full/terraform.tfstate.tflock") &&
      !strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab68/dev/full/terraform.tfstate")
    )
    error_message = "Plan role backend policy must be scoped to the lesson 70 state key and lockfile only."
  }

  assert {
    condition = (
      strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab70/dev/full/terraform.tfstate") &&
      strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab70/dev/full/terraform.tfstate.tflock") &&
      !strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab68/dev/full/terraform.tfstate")
    )
    error_message = "Apply role backend policy must be scoped to the lesson 70 state key and lockfile only."
  }
  ...
  assert {
    condition = anytrue([
      ...
      try(
        ...
        statement.Resource == "arn:aws:iam::123456789012:role/lab70-ec2-ssm-role" &&
        statement.Condition.StringEquals["iam:PassedToService"] == "ec2.amazonaws.com",
        false
      )
    ])
    error_message = "Apply role PassRole must be restricted to the lab70 EC2 runtime role and ec2.amazonaws.com."
  }
