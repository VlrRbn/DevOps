resource "aws_cloudwatch_metric_alarm" "function_throttles" {
  ...
  alarm_description   = "Lambda FIFO worker is being throttled"
...
resource "aws_cloudwatch_metric_alarm" "source_oldest_message" {
  ...
  alarm_description   = "FIFO source queue processing is blocked or falling behind"
  namespace           = "AWS/SQS"
...
resource "aws_cloudwatch_metric_alarm" "dead_letter_visible" {
  ...
  alarm_description   = "At least one message is visible in the FIFO DLQ"
  namespace           = "AWS/SQS"
