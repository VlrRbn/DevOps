...
### 1.5 Как читать `priority` в `journalctl`

Приоритеты (severity) идут от 0 до 7:
...
- `5` `notice` - важное, но не ошибка
- `6` `info` - информационные события
- `7` `debug` - отладочная детализация

Практически чаще всего:

- `-p warning` - показать предупреждения и все более серьезные события
- `-p err..alert` - только диапазон уровней ошибки/критичности
...
### 1.6 Мини-поток диагностики сервиса (быстрый triage)

Когда сервис ведет себя странно, иди в таком порядке:

1. `systemctl status <unit> --no-pager` - текущее состояние и последний error context.
2. `journalctl -u <unit> -n 50 --no-pager` - последние события по unit.
3. `journalctl -u <unit> --since "30 min ago" -p warning --no-pager` - отфильтрованные проблемы.
4. `systemctl cat <unit>` и `systemctl show -p ... <unit>` - что реально загружено и с какими параметрами.
5. При необходимости перезапусти unit и смотри live: `journalctl -fu <unit>`.

Это закрывает три главных вопроса: unit живой или нет, почему упал, и какая конфигурация реально применена.
