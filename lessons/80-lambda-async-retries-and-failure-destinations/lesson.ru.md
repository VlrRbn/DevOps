...
|---|---|---|
| `synchronous invocation` | синхронный вызов | Вызывающая сторона ждёт завершения handler и получает его ответ |
| `asynchronous invocation` | асинхронный вызов | Lambda принимает событие, а handler запускает отдельно от запроса отправителя |
| `Lambda asynchronous queue` | внутренняя асинхронная очередь Lambda | Управляемая AWS очередь принятых событий; пользователь не обращается к ней напрямую |
| `retry` | повторная попытка | Дополнительный запуск handler после неудачной обработки события |
| `event age` | возраст события | Время с момента приёма события сервисом Lambda |
| `failure destination` | место доставки записи об отказе | Ресурс, куда Lambda отправляет итоговую запись после исчерпания повторов |
| `dead-letter queue (DLQ)` | очередь необработанных сообщений | Очередь с исходным событием, которое Lambda не смогла обработать |
| `invocation record` | запись о вызове | Конверт с запросом, ответом, контекстом и сведениями о выполнении для destination |
| `event_id` | бизнес-ID события | Стабильный идентификатор события, который сохраняется между повторами |
| `RequestId` | ID попытки Lambda | Диагностический идентификатор выполнения, назначенный сервисом Lambda |
| `DestinationDeliveryFailures` | ошибки доставки в destination | Метрика CloudWatch для записей, которые Lambda не смогла передать в destination |
| `idempotency` | идемпотентность | Свойство обработки, при котором повтор события не создаёт повторный бизнес-эффект |
