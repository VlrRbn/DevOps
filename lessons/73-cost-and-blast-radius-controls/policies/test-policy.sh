...
TMP_ROOT="${TMPDIR:-/tmp}/l73-policy-tests_$$"
mkdir -p "$TMP_ROOT"
...
pass_case() {
  ...
  mkdir -p "$out_dir"
  OUT_DIR="$out_dir" "$POLICY" "$plan" >"/tmp/l73-policy-${name}.log"
...
deny_case() {
  ...
  mkdir -p "$out_dir"
  set +e
  OUT_DIR="$out_dir" "$POLICY" "$plan" >"/tmp/l73-policy-${name}.log" 2>&1
  local ec=$?
  set -e
  if [[ "$ec" -ne 2 ]]; then
    echo "Expected DENY exit code 2 for $name, got $ec" >&2
    cat "/tmp/l73-policy-${name}.log" >&2
    exit 1
  fi
...
pass_case_with_exception() {
  ...
  local out_dir="$TMP_ROOT/$name"
  mkdir -p "$out_dir"
  ALLOW_DESTROY_FILE="$exception" OUT_DIR="$out_dir" "$POLICY" "$plan" >"/tmp/l73-policy-${name}.log"
  grep -q 'POLICY_DECISION=ALLOW' "$out_dir/policy-decision.txt"
...
deny_case_with_exception() {
  ...
  mkdir -p "$out_dir"
  set +e
  ALLOW_DESTROY_FILE="$exception" OUT_DIR="$out_dir" "$POLICY" "$plan" >"/tmp/l73-policy-${name}.log" 2>&1
  local ec=$?
  set -e
  if [[ "$ec" -ne 2 ]]; then
    echo "Expected DENY exit code 2 for $name, got $ec" >&2
