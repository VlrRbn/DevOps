mock_provider "aws" {
  ...
  mock_resource "aws_lb" {
    defaults = {
      arn      = "arn:aws:elasticloadbalancing:eu-west-1:123456789012:loadbalancer/app/lab73-app-alb/test"
      dns_name = "internal-lab73-app-alb.example.local"
    }
  }

  mock_resource "aws_lb_target_group" {
    defaults = {
      arn = "arn:aws:elasticloadbalancing:eu-west-1:123456789012:targetgroup/lab73-web-tg/test"
    }
  }
...
variables {
  ...
  public_subnet_cidrs  = ["10.73.1.0/24", "10.73.2.0/24"]
  private_subnet_cidrs = ["10.73.11.0/24", "10.73.12.0/24"]
  ...
  tf_state_key         = "lab73/dev/full/terraform.tfstate"
}
...
run "iam_policy_contract" {
  ...
  assert {
    condition = (
      aws_iam_role.github_actions_plan_role.name == "lab73-github-actions-plan-role" &&
      aws_iam_role.github_actions_apply_role.name == "lab73-github-actions-apply-role"
    )
  ...
  assert {
    ...
    error_message = "Lesson 73 must not reintroduce broad AWS managed policies into the normal plan/apply roles."
  }

  assert {
    condition = (
      strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab73/dev/full/terraform.tfstate") &&
      strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab73/dev/full/terraform.tfstate.tflock") &&
      !strcontains(aws_iam_role_policy.github_actions_backend_access.policy, "lab68/dev/full/terraform.tfstate")
    )
    error_message = "Plan role backend policy must be scoped to the lesson 73 state key and lockfile only."
  }

  assert {
    condition = (
      strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab73/dev/full/terraform.tfstate") &&
      strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab73/dev/full/terraform.tfstate.tflock") &&
      !strcontains(aws_iam_role_policy.github_actions_apply_scoped.policy, "lab68/dev/full/terraform.tfstate")
    )
    error_message = "Apply role backend policy must be scoped to the lesson 73 state key and lockfile only."
  }
  ...
  assert {
    condition = anytrue([
      ...
      try(
        ...
        statement.Resource == "arn:aws:iam::123456789012:role/lab73-ec2-ssm-role" &&
        statement.Condition.StringEquals["iam:PassedToService"] == "ec2.amazonaws.com",
        false
      )
    ])
    error_message = "Apply role PassRole must be restricted to the lab73 EC2 runtime role and ec2.amazonaws.com."
  }
