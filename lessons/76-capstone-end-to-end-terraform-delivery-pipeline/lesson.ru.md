...
- `source_workflow_run_url` для `stage`/`prod` указывает на предыдущий successful run и проверяется через GitHub API;
- `promotion-manifest.json` из source apply artifact подтверждает source environment, `release_id`, commit SHA, successful apply и clean post-apply drift;
