...
validate source promotion-manifest.json
assume plan role
...
- `source_workflow_run_url` for `stage`/`prod` points to the previous successful run and is verified through the GitHub API;
- `promotion-manifest.json` from the source apply artifact confirms source environment, `release_id`, commit SHA, successful apply, and clean post-apply drift;
