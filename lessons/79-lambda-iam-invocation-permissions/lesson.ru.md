...
|---|---|---|
| `principal` | субъект IAM | Сущность, которой AWS разрешает или запрещает действие |
| `identity` | учётная сущность IAM | IAM user или IAM role с собственными разрешениями |
| `operator identity` | учётная сущность оператора | Пользователь или роль, от имени которой запускаются Terraform и AWS CLI |
| `caller role` | вызывающая роль | IAM role, от имени которой отправляется `lambda:InvokeFunction` |
| `execution role` | роль выполнения Lambda | IAM role, которую принимает Lambda для доступа функции к AWS API |
| `trust policy` | политика доверия | Определяет, кто может принять IAM role через STS |
| `identity-based policy` | политика учётной сущности | Разрешения, прикреплённые к IAM user или IAM role |
| `resource-based policy` | ресурсная политика | Разрешения, прикреплённые непосредственно к ресурсу, здесь к функции Lambda |
| `assume role` | принять роль | Получить через STS временные права указанной IAM role |
| `session credentials` | временные учётные данные сессии | `AccessKeyId`, `SecretAccessKey` и `SessionToken`, выданные STS |
| `explicit deny` | явный запрет | Решение `Deny`, которое имеет приоритет над любым `Allow` |
| `cross-account access` | доступ между аккаунтами | Доступ субъекта одного AWS account к ресурсу другого account |
